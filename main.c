	...
	for (int j = 0; j < 6; j++) {
		for (int i = 0; i < 6; i++) {
			if (i + 1 < 6 && j + 1 < 6) {
				...
				sfRenderWindow_drawVertexArray(window,create_line(&map_two_d[states[color]->shape.x][states[color]->shape.y], &map_two_d[states[color]->shape.x][states[color]->shape.y + 1]), NULL);
				sfRenderWindow_drawVertexArray(window,create_line(&map_two_d[states[color]->shape.x][states[color]->shape.y + 1], &map_two_d[states[color]->shape.x][states[color]->shape.y + 2]), NULL);
				sfRenderWindow_drawVertexArray(window,create_line(&map_two_d[states[color]->shape.x][states[color]->shape.y + 1], &map_two_d[states[color]->shape.x + 1][states[color]->shape.y + 1]), NULL);
				sfRenderWindow_drawVertexArray(window,create_line(&map_two_d[states[color]->shape.x - 1][states[color]->shape.y + 1], &map_two_d[states[color]->shape.x][states[color]->shape.y + 1]), NULL);
			}
		}
	}
...
int **analyse_events(sfRenderWindow *window, sfEvent event, states_t **states, int **map)
{
	...
	while (sfRenderWindow_pollEvent(window, &event)) {
			...
			if (event.type == sfEvtKeyPressed) {
				...
				if (event.key.code == sfKeyDown && states[color]->shape.x < 4)
					states[color]->shape.x ++;
				if (event.key.code == sfKeyLeft && states[color]->shape.y > 0)
					states[color]->shape.y --;
				if (event.key.code == sfKeyRight && states[color]->shape.y < 3)
					states[color]->shape.y ++;
				sfRenderWindow_clear(window, sfBlack);
			}
		}
