...
int	is_here_posi(sfVector2i *pos, sfVector2f **map, int x, int y)
{
...
sfVector2i	search_position_selectioned_square(world_t *world)
{
	...
	for (unsigned int y = 0; y != map->height - 1; y++) {
		for (unsigned int x = 0; x != map->width - 1; x++) {
			if (map->rotation < 0 && is_here_negatif(&pos,
							map->graph_map, x, y)) {
				pos.x = x;
				pos.y = y;
				return (pos);
			} else if (is_here_posi(&pos, map->graph_map, x, y)) {
				pos.x = x;
				pos.y = y;
				return (pos);
			}
...
sfVector2i	search_position_selectioned_summit(world_t *world)
{
	...
	for (unsigned int y = 0; y != world->map->height - 2; y++) {
		for (unsigned int x = 0; x != world->map->width - 2; x++) {
			if ((pos.x > map[y][x].x - 30 && pos.x <
map[y][x].x + 30) && (pos.y > map[y][x].y - 30 && pos.y < map[y][x].y + 30)) {
				pos.x = x;
				pos.y = y;
				return (pos);
			}
