...
void	create_button_menu(menu_t *menu)
{
	...
	set_helptextbox(&menu->button[0], menu->font, "CREATE MAP : width && height = number");
	set_button_texture(&menu->button[0], "./picture/create_idle.png", "./picture/create_hover.png", "./picture/validated.png");
	set_button(&menu->button[1], (sfVector2f){1000, 500}, &menu_load_map);
	set_helptextbox(&menu->button[1], menu->font, "LOAD MAP : give path in name");
	set_button_texture(&menu->button[1], "./picture/load_idle.png", "./picture/load_hover.png", "./picture/validated.png");
}
void	create_menu(menu_t *menu)
{
	...
	menu->tex_back = sfTexture_createFromFile("./picture/background_menu.png", NULL);
